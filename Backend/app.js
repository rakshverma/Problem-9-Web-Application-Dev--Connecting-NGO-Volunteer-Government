  ...
  app.get("/api/projects/recent", async (req, res) => {
    try {
      const projects = await Project.find()
        ...
        .limit(3) // Get only 3 projects
        .populate("organization", "username"); // Populate organization name
      res.json(projects);
    } catch (error) {
      res.status(500).json({ message: "Server error", error });
    }
...
app.delete("/api/projects/:projectId", authenticateToken, async (req, res) => {
  try {
    ...
    // ✅ Only allow NGO, government, or admin to delete the project
    if (
      req.user.role !== "admin" &&
      project.organization.toString() !== req.user.id
    ) {
      return res
        .status(403)
